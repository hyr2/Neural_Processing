...
input_dir_list = dir_sorted(input_dir);
input_dir_list(1) = [];                 % removing the dot directories (linux)
input_dir_list(1) = [];
input_dir_list(end) = [];               % removing the extras directory


for iter_local = 1:size(input_dir_list,1)
    
    
    source_dir = input_dir_list(iter_local).name;
    source_dir = fullfile(input_dir,source_dir);
    datestr = '2021-12-09';
    % Set these parameters -----------all parameters in seconds *****
