...
# -------------------------settings
F_SAMPLE = 25e3 # check rhd file or the info.mat from preprocessing rhd file
GH = 25
GW_BETWEENSHANK = 300
GW_WITHINSHANK  = 25
RASTER_PLOT_AMPLITUDE = False
FIGDIRNAME = "figs_allclus_waveforms"
CHANNELMAP2X16 = True # this affects how the plots are generated
session_folder = r"D:\Rice-Courses\neuroeng_lab\codes\stroke_proj_postproc\data_msort\NVC\BC7\12-24-2021\ePhys\Processed\msorted"
session_newsavefolder = r"D:\Rice-Courses\neuroeng_lab\codes\stroke_proj_postproc\data_msort\NVC\BC7\12-24-2021\ePhys\Processed\msorted"
WINDOW_LEN_IN_SEC = 15 # 30e-3
SMOOTHING_SIZE = 5
WINDOW_IN_SAMPLES = int(WINDOW_LEN_IN_SEC*F_SAMPLE)
NO_READING_FILTMDA = True # set to False on first run of each session
N_PROCESSES = 1 # multiprocessing option
...
COMMON_AVG_REREFERENCE = False
ADJACENCY_RADIUS_SQUARED = 140**2 # um^2, [consistent with mountainsort shell script](not anymore) # potential bending in B-BC5 ?
SNR_THRESH = 1.5
AMP_THRESH = 50
...
def postprocess_one_session(session_folder_load, session_folder_save):
    ...
    amp_thresh = AMP_THRESH # 50 # in uV
    ...
    firing_rate_thresh = FIRING_RATE_THRESH # 0.05
    ...
    for i_clus in tmp_clus_ids:
        ...
        p2p_near = p2p_by_channel>P2P_PROPORTION_THRESH*p2p_prim
    ...
    # reject by amplitude histogram shape
    kurtosis = np.zeros(n_clus)
    # skewness = np.zeros(n_clus)
    for i_clus in range(n_clus):
        kurtosis[i_clus] = stats.kurtosis(spk_amp_series[i_clus], fisher=True, bias=True)
    # cluster_accept_mask[np.logical_or(kurtosis>3, kurtosis<-0.5)] = False
    print(kurtosis)
    # print("%d/%d clusters kept after amplitude-distribution screening"%(np.sum(cluster_accept_mask), n_clus))


    # reject by 2ms-ISI violation ratio of 1%
    multi_unit_mask = np.logical_and(cluster_accept_mask, refrac_violation_ratio > 0.01)
    pd.DataFrame(data=cluster_accept_mask.astype(int)).to_csv(os.path.join(session_folder_save, "accept_mask.csv"), index=False, header=False)
    pd.DataFrame(data=multi_unit_mask.astype(int)).to_csv(os.path.join(session_folder_save, "multi_unit_mask.csv"), index=False, header=False)
    ...
    def plot_single_cluster_1x32(i_clus_plot):
        ...
        ax_template = fig2.add_subplot(gs_ovr[15:21, 24:])
        ax_template.plot(\
            np.arange(waveform_len)/F_SAMPLE*1000, \
        ...
        # print annotations
        ax_text = fig2.add_subplot(gs_ovr[23:, 24:])
        ...
        str_annot += "Automatic screening: %s\n" % ("passed" if cluster_accept_mask[i_clus_plot] else "failed")
    ...
    def plot_single_cluster_2x16(i_clus_plot):
        ...
        text_str = "Mean=%.2fuV\nStd=%.2fuV\nMin=%.2fuV\nMax=%.2fuV\nBinwidth=%.2fuV\nKurtosis=%.2f" % ( \
            amp_mean, amp_std, amp_min, amp_max, amphist_binwidth, kurtosis[i_clus_plot])
        ...
        plt.subplots_adjust(wspace=0.05, hspace=0.05)
        if cluster_accept_mask[i_clus_plot]:
            print(os.path.join(figpath, "waveform_clus%d.png"%(clus_labels[i_clus_plot])))
            plt.savefig(os.path.join(figpath, "waveform_clus%d.png"%(clus_labels[i_clus_plot])))
        elif multi_unit_mask[i_clus_plot]:
            print(os.path.join(figpath, "z_multiunit_waveform_clus%d.png"%(clus_labels[i_clus_plot])))
            plt.savefig(os.path.join(figpath, "z_multiunit_waveform_clus%d.png"%(clus_labels[i_clus_plot])))
        else:
            print(os.path.join(figpath, "z_failed_waveform_clus%d.png"%(clus_labels[i_clus_plot])))
            plt.savefig(os.path.join(figpath, "z_failed_waveform_clus%d.png"%(clus_labels[i_clus_plot])))
        plt.close()
        print(i_clus_plot+1)
