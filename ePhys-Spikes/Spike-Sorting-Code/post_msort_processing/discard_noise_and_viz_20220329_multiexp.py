...
# F_SAMPLE = 20e3 # BC6 first few experiments
RASTER_PLOT_AMPLITUDE = False
FIGDIRNAME = "figs_allclus_waveforms"
CHANNELMAP2X16 = False # this affects how the plots are generated
SESSION_FOLDER = "/media/luanlab/Data_Processing/Jim-Zhang/Spike-Sort/spikesort_out/NVC/BC7/combined0"
session_newsavefolder = os.path.join(SESSION_FOLDER, "Postproc20220329")
WINDOW_LEN_IN_SEC = 20e-3 # 30e-3
SMOOTHING_SIZE = 5
...
FIRING_RATE_THRESH = 0.1 # 0.5
P2P_PROPORTION_THRESH = 0.3
PARAMS = {}
...
def postprocess_one_session(session_folder_load, session_folder_save):
    ...
    # not doing it because file is too large (multiple experiments sorted together)
    firing_rate_series = []
    for i_clus in range(n_clus):
        firing_rate_series.append(single_cluster_firing_rate_series(spike_times_by_clus[i_clus]))
    ...
    def plot_single_cluster_1x32(i_clus_plot):
        ...
        ax_ampstamp.plot(np.arange(firing_rate_series[i_clus_plot].shape[0])*WINDOW_LEN_IN_SEC, firing_rate_series[i_clus_plot], linewidth=1, c='k')
        ax_ampstamp.set_xlim(0, firing_rate_series[i_clus_plot].shape[0]*WINDOW_LEN_IN_SEC)
        ax_ampstamp.set_xlabel("Time (sec)")
        ax_ampstamp.set_ylabel("Firing rate")
        
        # waveforms at primary channel for most events
        # not doing it

        # spike amplitude histogram
        # not doing it because can count all (files too large)
