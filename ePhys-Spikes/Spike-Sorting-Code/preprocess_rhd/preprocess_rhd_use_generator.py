...
notch_freq = None
sample_freq = None
for (filename, data_dict) in ((filename, read_data(os.path.join(SESSION_FOLDER_RAW, filename))) for filename in filenames):
    ...
    # data_rhd_list.append(ephys_data)

# ephys_data_whole = data_rhd_list[0]
# for ephys_data in data_rhd_list[1:]:
#     ephys_data_whole = np.concatenate([ephys_data_whole, ephys_data], axis=-1)
#     del(ephys_data)
#     gc.collect()
# ephys_data_whole = np.concatenate(data_rhd_list, axis=-1) # concatenate horizontally
# print("Applying notch")
# ephys_data_whole = notch_filter(ephys_data_whole, sample_freq, notch_freq, Q=20)
