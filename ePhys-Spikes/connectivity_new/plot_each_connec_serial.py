...
def plot_each_connec_(pp_dict_, figsavedir):
    """
        Given relevant data of a session (pp_dict_), for each putative connection pair:
    ...
    for i_connec in range(connecs.shape[0]):
        print("   ", i_connec)
...
def proc_main(spk_dir_root, mda_tempdir_root, connec_dir_root, spk_reldirs, mda_reldirs):
    ...
    for mda_reldir, spk_reldir in zip(mda_reldirs, spk_reldirs):
        ...
        parallel_args.append((pp_dict, session_con_dir))
