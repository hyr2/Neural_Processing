...
def read_postproc_data(session_spk_dir: str, mdaclean_temp_dir: str, session_connec_dir: str, shank_def: dict, apply_curation: bool, count_type: int) -> dict:
    ...
    try:
        hu, au = nx.hits(g, normalized=True)
        ret["graph_hits_h_array"] = np.array(list(hu.values()))
        ret["graph_hits_a_array"] = np.array(list(au.values()))
        print("ghits h:", ret["graph_hits_h_array"].shape)
        print("ghits a:", ret["graph_hits_a_array"].shape)
    except:
