function [mono_res, connectivityGraph, connectivityCount] = core_CE_connec(...
    firings, templates, idx, Fs, GW_BETWEENSHANK, GH, CHMAP_2X16, ...
    ...
    tic
    mono_res = ce_MonoSynConvClick(spikes,'binSize',para_connect.binSize,'sigWindow',para_connect.sigWindow,'conv_w',para_connect.conv_w,'duration',para_connect.duration); %,'includeInhibitoryConnections',false); % detects the monosynaptic connections
    toc
    ...
    % count statistics
    connectivityCount = zeros(4,4);     % For 4 shank device
    ...
    %     disp(src_shanknum)
        ...
        connectivityCount(src_shanknum,snk_shanknum) = connectivityCount(src_shanknum,snk_shanknum) + 1;
    end
    ...
    savefig(fullfile(resultfolder,sprintf('connec_%s.fig', figname)));
    % savefig('D:\Rice-Courses\neuroeng_lab\codes\stroke_proj_connec\plots\NVC\BC7\01-21-2022.fig')
    saveas(gcf, fullfile(resultfolder,sprintf('connec_%s.png', figname)));
    % figure
