...
GW_BETWEEN_SHANK = None
GH = None
GW_WITHIN_SHANK = None

def gen_geommap(chmap_mat,list_channels):
    global GW_BETWEEN_SHANK, GH, GW_WITHIN_SHANK
...
def gen_1x32(geom_np):
    global GW_BETWEEN_SHANK, GH, GW_WITHIN_SHANK
    loc = np.zeros([32,4])  # this is the 1x32 four channel
    for i, (x,y) in enumerate(zip(geom_np[:,0],geom_np[:,1])):
        shank_ID_loc1 = x // GW_BETWEEN_SHANK
        row_ID = y//GH
        loc[row_ID,shank_ID_loc1] = 1  # binary mask for channels to be accepted
...
# input_dir = '/home/hyr2-office/Documents/Data/NVC/BC7/'
# channel_map_path = '/home/hyr2-office/Documents/git/Neural_SP/Neural_Processing/Channel_Maps/chan_map_1x32_128ch_flex_new.mat'

def find_min_chan_map(input_dir,channel_map_path,electrode_2x16):
    global GW_BETWEEN_SHANK, GH, GW_WITHIN_SHANK
    ...
    #     a.plot(geom_map_orig[:,0],-geom_map_orig[:,1],'o')
    #     a.plot(gen_geommap(chan_map,min_chanmap_mask_list[iter])[:,0],-gen_geommap(chan_map,min_chanmap_mask_list[iter])[:,1],'x')
