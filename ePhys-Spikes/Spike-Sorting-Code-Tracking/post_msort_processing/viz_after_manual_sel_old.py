...
animal_folder = "/media/luanlab/Data_Processing/Jim-Zhang/SpinalCordSpikeSort/msort_outputs/Tacoma_11142021_IrOx_Allrefsout/"
# session_folder = "/media/luanlab/Data_Processing/Jim-Zhang/SpinalCordSpikeSort/msort_outputs/Nacho_090721_1um_4refOut_600um/nacho_awake_210910_211048"
...
def plot_multiple_clusters(final_stamp_time, n_clus, geom, clus_coordinates, cmap, smap, \
    peak_amplitude_ranks, pri_ch_lut, spike_times_by_clus, spk_amp_series, plot_mask, fig_fpath):
    
    '''viz cluster locations'''

    # if plot_mask.dtype != bool:
    #     plot_mask_this = plot_mask.astype(bool)
    # else:
    #     plot_mask_this = plot_mask
    clus_coordinates_this = clus_coordinates[plot_mask.astype(bool), :]
    fig1 = plt.figure(figsize=(7, 16))
    peak_amplitude_ranks_this = peak_amplitude_ranks[plot_mask.astype(bool)]
    gs_ovr = gridspec.GridSpec(16,7, figure=fig1)
...
def main_postprocess_and_viz(session_folder):
    ...
    n_bins=50
    isi_bin_edges = np.linspace(0, 50, n_bins+1) # in millisec; 1ms per bin
    ...
    plot_multiple_clusters(final_stamp_time, n_clus, geom, clus_coordinates, cmap, smap, \
        peak_amplitude_ranks, pri_ch_lut, spike_times_by_clus, spk_amp_series, multi_mask, fig_fpath_multi)
    
    fig_fpath_single = os.path.join(figpath_single, "all_locations_single.png")
    plot_multiple_clusters(final_stamp_time, n_clus, geom, clus_coordinates, cmap, smap, \
        peak_amplitude_ranks, pri_ch_lut, spike_times_by_clus, spk_amp_series, single_mask, fig_fpath_single)
    ...
    for i_clus_plot in range(n_clus):
        ...
        # plot ISI histogram
        ax_isihist = fig2.add_subplot(gs_ovr[:4, 10:])
        ...
        ax_ampstamp = fig2.add_subplot(gs_ovr[5:9, 10:])
        ...
        # print annotations
        ax_text = fig2.add_subplot(gs_ovr[10:, 10:])
