...
# 			psd_bl = np.reshape(psd_bl,(psd_bl.size,1))
	...
	t_activation_MUA_post = np.where(np.logical_and(t_stft>=stim_end_time-time_window, t_stft<=stim_end_time+time_window))
	t_activation_MUA_post = np.asarray(t_activation_MUA_post)
	t_activation_MUA_post = np.reshape(t_activation_MUA_post,(t_activation_MUA_post.size,))
	MUA_depth_mean_post[chan_loc[0],chan_loc[1]] = np.mean(spectrogram_ndPSD_MUA[t_activation_MUA_post[0:]])
	...
	a0.set_xlim([time_start_spectrogram,time_limit_spectrogram-3.25])
	a1.plot(Time,ADC_data)
	a1.set_xlim([time_start_spectrogram,time_limit_spectrogram-3.25])
	...
	a2.set_xlim([time_start_spectrogram,time_limit_spectrogram-3.25])
	a3.plot(Time,ADC_data)
	a3.set_xlim([time_start_spectrogram,time_limit_spectrogram-3.25])
	...
	LL = len(f_cwt)
	LL_x = len(Time_mwt)
	x_tick_list = np.array(np.arange(0,10*Fs/decimate_factor,1*Fs/decimate_factor),dtype = np.int16)
	x_label_list = np.array(np.arange(Time_mwt[0],10,1),dtype = np.int16)
	...
	fg.suptitle(title_str)
	max_lim = np.mean(spectrogram_ndPSD) + 3*np.std(spectrogram_ndPSD)
	min_lim = np.mean(spectrogram_ndPSD) - 1*np.std(spectrogram_ndPSD)
	...
	filename_save = 'cwt-' + str(iter_chan) + '.png'
	...
	a11.plot(t_stft,spectrogram_ndPSD_MUA,'k', lw = 1.75)
	a7.legend([r'$\alpha$'])
	a8.legend([r'$\beta$'])
	a9.legend([r'$\gamma$'])
	a10.legend(['LFP-high'])
	a11.legend(['MUA'])
	a7.set_xlim([time_start_spectrogram,time_limit_spectrogram-3.25])
	a8.set_xlim([time_start_spectrogram,time_limit_spectrogram-3.25])
	a9.set_xlim([time_start_spectrogram,time_limit_spectrogram-3.25])
	a10.set_xlim([time_start_spectrogram,time_limit_spectrogram-3.25])
	a11.set_xlim([time_start_spectrogram,time_limit_spectrogram-3.25])
	...
	filename_save = 'Average-change-PSD-LFP' + str(iter_chan) + '.png'
	...
	data_summary = {'empty':[0],'time_MUA':t_stft,'time_LFP':Time_mwt,'freq_LFP':f_cwt,'LFP_ndPSD':spectrogram_ndPSD,'MUA_ndPSD':spectrogram_ndPSD_MUA,'Alpha_ndPSD':avg_ndPSD_Alpha,'Beta_ndPSD':avg_ndPSD_Beta,'Gamma_ndPSD':avg_ndPSD_Gamma,'LFPhigh_ndPSD':avg_ndPSD_high,'decimate_factor':decimate_factor}
...
data_summary = {'empty':[0],'depth_shank':depth_shank,'MUA_depth_peak':MUA_depth_peak,'MUA_depth_mean':MUA_depth_mean,'LFP_depth_peak':LFP_depth_peak,'LFP_depth_mean':LFP_depth_mean,'MUA_depth_mean_post':MUA_depth_mean_post,'LFP_depth_mean_post':LFP_depth_mean_post}
savemat(filename_save,data_summary)
