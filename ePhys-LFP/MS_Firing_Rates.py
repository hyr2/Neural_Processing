...
import pandas as pd
from Support import read_stimtxt
from Support_MS import *


# Files and Folders
source_dir = input('Input source_dir:\n')
# source_dir = '/run/media/hyr2/Data_OverflowB/BC6/2021-12-14/ePhys/'
CHANNEL_MAP_FPATH = os.path.join(source_dir,'chan_map_1x32_128ch_rigid.mat')
...
chan_map_knob = 1               # For 1x32 channel mapping

# Channel mapping
if (chan_map_knob == 1):     # 1x32 channel map
    dir_chan_map = os.path.join(source_dir,'chan_map_1x32_128ch.xlsx')
    df_chanMap = pd.read_excel(dir_chan_map,usecols=[0,1,2,3],header = None,sheet_name = 2)             # 4 shank device 1x32
    GH = 25
    GW_BWTWEENSHANKS = 250
    arr_chanMap = df_chanMap.to_numpy()    
elif (chan_map_knob == 2):  # 2x16 channel map
    dir_chan_map = os.path.join(source_dir,'chan_map_2x16_128ch.xlsx') 
    df_chanMap = pd.read_excel(dir_chan_map,usecols=[0,1,2,3,4,5,6,7],header = None,sheet_name = 2)     # 4 shank device 2x16
    GH = 30
    GW_BWTWEENSHANKS = 250
    arr_chanMap = df_chanMap.to_numpy()    
    
chmap_mat = arr_chanMap     
...
for i in range(n_ch_this_session):
    ...
    x, y = geom[i,:]
    # i_shank = int(x/GW) 
    prim_ch_intan = get_intan_from_coordinate(x,y,chan_map_knob) # the real index in the complete 128 channel map
...
#     plt.axvline(STIM_START_TIME_PLOT+STIM_DURATION chmap_mat = loadmat(CHANNEL_MAP_FPATH)['Ch_Map_new']N, linestyle='--', color='r')
