...
%         HbT(iter_seq,:,:) = HbO(iter_seq,:,:) + HbR(iter_seq,:,:);
    ...
    fcuthigh = 0.1;    % cutoff frequency
    Fs = 1/0.18;        % Sampling frequency
    pass_filter = designfilt('highpassiir','FilterOrder',order, ...
        'StopbandFrequency',fcuthigh,'SampleRate',Fs,'DesignMethod','cheby2');
    HbR(isnan(HbR)) = 0;
    HbO(isnan(HbO)) = 0;
    HbO = filtfilt(pass_filter,HbO);
    HbR = filtfilt(pass_filter,HbR);
    HbT = HbO + HbR;
    ...
    % Plotting
    plotter_Hb_conc(HbO,output_plot,'HbO',[-20 20]);
    plotter_Hb_conc(HbR,output_plot,'HbR',[-10 10]);
    plotter_Hb_conc(HbT,output_plot,'HbT',[-15 10]);
    fg = imshow(HbO_pk,[-10 20]);
    colormap('jet');
    colorbar;
    name_iter = fullfile(output_plot,strcat('pk-response-HbO','.svg'));
    saveas(fg,name_iter,'svg');
    fg = imshow(HbR_pk,[-10 10]);
    colormap('jet');
    colorbar;
    name_iter = fullfile(output_plot,strcat('pk-response-HbR','.svg'));
    saveas(fg,name_iter,'svg');
    fg = imshow(HbT_pk,[-15 10]);
    colormap('jet');
    colorbar;
