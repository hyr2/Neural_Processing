...
num_animals = 6;    % number of animals
...
shift_vec_early = [out_rh3{5}(find(out_rh3{4} < 21))',out_rh7{5}(find(out_rh7{4} < 21))',out_rh8{5}(find(out_rh8{4} < 21))',out_rh9{5}(find(out_rh9{4} < 21))',out_rh11{5}(find(out_rh11{4} < 21))'];
shift_vec_chronic = [out_rh3{5}(find(out_rh3{4} > 21))',out_rh7{5}(find(out_rh7{4} > 21))',out_rh8{5}(find(out_rh8{4} > 21))',out_rh9{5}(find(out_rh9{4} > 21))',out_rh11{5}(find(out_rh11{4} > 21))'];
...
%% Statistical tests
[h,p] = ttest2(shift_vec_early,shift_vec_chronic);
string_out = sprintf('The shift in cortical activity map has a p-value of: %.4f', p);
disp(string_out);

%% Plots for Figure1G (inter-day shift):
histogram(shift_vec_early, 'BinWidth', 0.312,'FaceColor', [0.1, 0.9, 0.9],'EdgeColor',[0.08, 0.65, 0.9]);
hold on;
histogram(-shift_vec_chronic, 'BinWidth', 0.312,'FaceColor', [0.9, 0.1, 0.2],'EdgeColor',[1.0, 0.05, 0.1]);
xlim([-2,2.1])
...
arr_area_all(4,:) = out_rh9{6}; % Areas are messed up. Check this
arr_area_all(5,:) = out_rh11{6};
arr_area_all(6,:) = out_bc7{6};     % day 7 is messed up
nanmean(arr_area_all,1)

% out_rh3{6} = helper_fill_nans(xaxis_ideal,xaxis_rh3(2:end),out_rh3{6});
% out_rh3{6} = helper_fill_nans(xaxis_ideal,xaxis_rh3(2:end),out_rh3{6});
% out_rh3{6} = helper_fill_nans(xaxis_ideal,xaxis_rh3(2:end),out_rh3{6});
% out_rh3{6} = helper_fill_nans(xaxis_ideal,xaxis_rh3(2:end),out_rh3{6});
% out_rh3{6} = helper_fill_nans(xaxis_ideal,xaxis_rh3(2:end),out_rh3{6});
