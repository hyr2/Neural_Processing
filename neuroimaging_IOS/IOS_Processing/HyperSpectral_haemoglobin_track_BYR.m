function [im_delta_c] = HyperSpectral_haemoglobin_track_BYR(im_blue,im_yellow,im_red)
...
% green: 532 nm / 10 nm (e_HbO: 43876, e_HbR: 40584, X: 0.0338528)
% yellow: 580 nm / 10 nm (e_HbO: 50104, e_HbR: 37020, X: 0.0306261)
% red: 632 nm / 10 nm (e_HbO: 561.2, e_HbR: 4930.8, X: 0.3924145)
% green: 510 nm / 10 nm (e_HbO: 20035.2, e_HbR : 25773.6, X: 0.0712537)

img_size = size(im_red);
...
delta_mu_a_red = -1/X_red * log(im_red);

A = [e_HbO_blue, e_HbR_blue;
     e_HbO_yellow, e_HbR_yellow;
     e_HbO_red, e_HbR_red ];

% Applying least-squares on each pixel
for x = [1:img_size(1)]
    for y = [1:img_size(2)]
...
%         b = leastSquare_direct(A,Y);
        b = A\Y;      % least-squares approximation (matlab function)
        im_delta_c(x,y,:) = b;
