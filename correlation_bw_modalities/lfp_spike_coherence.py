...
def read_spiking(folderpath):
    ...
    pri_chs = np.full(n_clus, -1)
    spike_times_by_clus =[[] for i in range(n_clus)]
    for spk_time, spk_lbl, pri_ch in zip(spike_times_all, spike_labels, firings[0,:]):
        spike_times_by_clus[spk_lbl-1].append(spk_time-1)
        if pri_chs[spk_lbl-1] == -1:
            pri_chs[spk_lbl-1] = pri_ch
    ...
    return spike_stamps, map_curation2msort, unit_locs, pri_chs
...
def coherence_one_session(session_spk_dir, session_lfp_dir):
    ...
    ##### TRIAL REJETION BY EPHYS
    print("#trials before ePhys noise rejection:", np.sum(trial_mask))
    trial_mask_temp = utils_cc.reject_trials_by_ephys(trials_data, 900, [2,4], ephys_newfreq)
    ...
    print("Resampled frequency:", ephys_newfreq, " bin_len:", bin_len)
    spike_stamps, map_curation2msort, unit_locs, pri_chs = read_spiking(session_spk_dir)
    ...
    n_rfft = n_samples//2 + 1# ((n_samples/2+1) if (n_samples%2==0) else ((n_samples+1)/2))
    coherences = np.empty((spiking_timeseries.shape[0], n_rfft), dtype=float)
    rfftfreq = scipy.fft.fftfreq(n_samples, bin_len)[:n_rfft]
    for i_unit in range(spiking_timeseries.shape[0]):
        i_ch = pri_chs[i_unit]
        sig_a_by_trial = data_r[i_ch, :, :]
        sig_b_by_trial = spiking_timeseries[i_unit, :, :]
        spectra_a = scipy.fft.rfft(sig_a_by_trial, axis=-1) # (n_trials, n_rfft)
        ...
        coherences[i_unit, :] = crospectrum_ab_avg/(np.sqrt(sqrspectrum_a_avg*sqrspectrum_b_avg))
    return coherences, rfftfreq, pri_chs
...
if __name__ == "__main__":
    ...
    fig_dir = "/media/hanlin/Liuyang_10T_backup/jiaaoZ/128ch/spikeSorting128chHaad/tmp_figs/bc7_211206_prilfp_spk_coherence_fs300hz_230310"
    ...
    for session_rel_dir in session_rel_dirs:
        ...
        coherences, rfftfreq, pri_chs = coherence_one_session(session_spk_dir, session_lfp_dir)
        ...
        for i_unit in range(coherences.shape[0]):
            i_ch = pri_chs[i_unit]
            plt.figure()
            plt.bar(rfftfreq, coherences[i_unit, :], width=rfftfreq[1]-rfftfreq[0])
            ...
            plt.xlabel("Freq (Hz)")
            plt.title("Coherence - spiking unit %d -vs- pri LFP ch %d " % (i_unit, i_ch))
            # plt.show()
