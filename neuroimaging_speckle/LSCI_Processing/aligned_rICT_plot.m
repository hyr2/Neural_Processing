function aligned_rICT_plot(file_aligned,varargin)
%aligned_rICT_plot Generates timecourse plots from aligned speckle contrast data
% aligned_rICT_plot(file_aligned) generates speckle contrast (SC), correlation 
% time (CT), relative inverse correlation time (rICT), and infarct size plots 
% with user-defined ROIs from aligned speckle contrast data. The rICT is 
...
%
% aligned_rICT_plot(file_aligned, file_sc_baseline) loads separate directory 
% of speckle contrast data for the rICT baseline.
%
%   file_sc_baseline = Path to directory of speckle contrast files
%
% aligned_rICT_plot(file_aligned, file_sc_baseline,ROI) use previously-
% defined ROI masks either loaded from a directory containing exported ROI 
% images from the Speckle Software (*_ccd.bmp format) or passed as an array
% of binary masks. The file_sc_baseline parameter can be empty (e.g. '' or []).
%
...
%
% aligned_rICT_plot(___,Name,Value) modifies the data processing using one or more
% name-value pair arguments.
%
%   'sc_range': Set display range for speckle contrast data (Default = [0 0.2])
%       Used when generated ROI overlays
%
%   'scale': Set imaging system resolution in pixels/mm (Default = 335)
%       Used when calculating the infarct area. Defaults to the multimodal
%       imaging system resolution.
%
%   'core_threshold': Set rICT threshold for ischemic core (Default = 0.2)
...
addParameter(p, 'sc_range', [0 0.7], validateFnc);
addParameter(p, 'scale', 335, @isscalar);
addParameter(p, 'core_threshold', 0.2, @isscalar);
parse(p, file_aligned, varargin{:});
...
SC_END = loadAlignedSC(files(end));
...
output = fullfile(pwd, ['rICT_2_' d_parts{end}]);
if ~exist(output,'dir')
  mkdir(output);
end
...
fprintf('Figures and data saved in %s\n', output);
