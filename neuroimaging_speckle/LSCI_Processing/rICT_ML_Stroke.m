function rICT_ML_Stroke(file_sc,n_avg,varargin)
%rICT_ML_Stroke Generates timecourse plots from speckle contrast data
% rICT_ML_Stroke(file_sc,n_avg) generates speckle contrast (SC), correlation 
% time (CT), relative inverse correlation time (rICT) plots with 
% user-defined ROIs from speckle contrast data. The rICT is 
...
addOptional(p, 'ROI', '');
addOptional(p, 'n_avg_bl', 20 ,@isscalar);
validateFnc = @(x) validateattributes(x, {'numeric'}, {'numel', 2, 'increasing'});
addParameter(p, 'sc_range', [0.02 0.35], validateFnc);
...
t = t(1:N_total);             % hardcoded for number of frames available
...
%     n_avg_bl = 5;
  SC_REF = squeeze(mean(read_subimage(files, -1, -1, [1:n_avg_bl]), 3))';
else
...
%   AREA(i) = length(find(CT_REF./CT_FRAME < core_threshold))/(scale^2);
  ...
  for j = 1:N_ROI
      ...
      SC_FRAME_ROI = ROI_NaN(:,:,j).*SC_FRAME; %#ok<PFBNS>
%       SC_REF_ROI = ROI_NaN(:,:,j).*SC_FRAME; %#ok<PFBNS>
      SC(i,j) = nanmean(SC_FRAME_ROI(:));
...
xlabel('Time (s)'); ylabel('rICT'); grid on;
title('Time-Series rICT');
legend(names);
print(fullfile(output, 'Time-Series rICT.png'), '-dpng');
...
save(fullfile(output, 'data.mat'),'rICT','t','ROI','t_start','names');
