function rICT_frame_binned(file_sc,sc_range,varargin)
%rICT_frame Generates single rICT frame
% rICT_frame_binned(file_sc,sc_range) generates an image overlaying relative 
...
%
% rICT_frame_binned(file_sc,sc_range,file_sc_baseline) loads separate directory of
...
addParameter(p, 'use_divergent_cmap', true, @islogical);
...
%     rICT = CT1./CT2;
    ...
    % Generatre the overlay alpha mask excluding values outside overlay range
    ...
    filename = fullfile(pwd, [d_parts{end},'_rICT',num2str(j)]);
