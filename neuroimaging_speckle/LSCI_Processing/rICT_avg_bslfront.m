function rICT_avg_bslfront(file_sc,len_trials,num_bin,trials,sc_range,varargin)
...
for k = 1:trials
    iter_bsl = [start_trial(k):start_trial(k)+num_BSL-1];   % Modified from rICT_avg.m Takes first num_BSL sequences as baseline
    for iter = iter_bsl
