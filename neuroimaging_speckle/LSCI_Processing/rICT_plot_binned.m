function rICT_plot_binned(file_sc,n_avg,varargin)
%rICT_plot_binned Generates timecourse plots from speckle contrast data
% rICT_plot_binned(file_sc,n_avg) generates speckle contrast (SC), correlation 
...
%
% rICT_plot_binned(file_sc,n_avg,file_sc_baseline,ROI) use previously-defined ROI 
...
%
% rICT_plot_binned(___,Name,Value) modifies the data processing using one or more
...
addParameter(p, 'scale', 355, @isscalar);
...
% N_total, files(1).folder, range(t));  % uncommend this line
fprintf('Identified %d frames in %s collected\n', N_total);  % delete this line
...
output = fullfile(pwd, ['rICT_','ROI_SCavg']);
if ~exist(output,'dir')
  mkdir(output);
end

% *** Temporary
t = [0:0.230:0.230*(length(files)-1)];
